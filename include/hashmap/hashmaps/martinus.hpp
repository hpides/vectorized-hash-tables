...
class MartinusRHFlatHashTable : public HashTable<KeyT, ValueT> {
 public:
  MartinusRHFlatHashTable(uint64_t max_elements, uint8_t /*target_load_factor*/, bool print_info = true,
                          std::string base_identifier = "MartinusRHFlatHashTable")
  ...
  uint64_t find(const KeyT& /*key*/) {
    spdlog::error("find is not implemented for martinus robin hood");
 ...
 protected:
  alignas(utils::cacheline_size) robin_hood::unordered_flat_map<KeyT, ValueT, utils::ExternalHashWrapper<KeyT, HasherT>> map_;
