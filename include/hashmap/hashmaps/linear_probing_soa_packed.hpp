...
class LinearProbingPackedSoAHashTable : public HashTable<KeyT, ValueT> {
 public:
  LinearProbingPackedSoAHashTable(uint64_t max_elements, uint8_t /*target_load_factor*/, bool print_info = true,
                                  std::string base_identifier = "LinearProbingPackedSoAHashTable")
      : keys_(max_elements), values_(max_elements), max_elements_{max_elements}, size_{0}, hasher_(max_elements), base_identifier_{base_identifier} {
    if (print_info) {
  ...
  void insert(const KeyT& key, const ValueT& value) {
    ...
    if (!res.key_equal || !res.entry_valid) {
      ...
      ++size_;
      keys_[res.idx] = PackedSoAKey<KeyT>(key);
    }
  ...
  FindResult find(const KeyT& key) {
    ...
    PackedSoAKey<KeyT> next_key = keys_[curr_adjusted_idx];

    bool key_not_equal = next_key.key != key;

    auto loop_body = [&]() {
      curr_adjusted_idx = hasher_.finalize(++curr_idx);
      next_key = keys_[curr_adjusted_idx];
      key_not_equal = next_key.key != key;
    };

    while (key_not_equal && next_key.is_valid && curr_idx <= max_idx) {
      if constexpr (unroll_factor == 1) {
        ...
        utils::loop<uint8_t, unroll_factor>([&](auto i) {
          ...
          if constexpr (i == 0) {
            ...
            if (key_not_equal && next_key.is_valid && curr_idx <= max_idx) {
              loop_body();
            }
          }
        });
    ...
    return {curr_adjusted_idx, next_key.key, !key_not_equal, next_key.is_valid};
  }
  ...
  void prefault() {
    ...
    for (uint64_t i = 0; i < keys_.size(); i++) {
      if constexpr (!std::is_same_v<KeyT, StringKey>) {
        keys_[i] = PackedSoAKey<KeyT>(static_cast<KeyT>(dist(gen)));
      }
      if constexpr (!std::is_pointer_v<ValueT>) {
  ...
  void prefault_pregenerated(const std::vector<std::pair<KeyT, ValueT>>& prefault_data) {
    ...
    for (uint64_t i = 0; i < keys_.size(); i++) {
      keys_[i] = PackedSoAKey<KeyT>(prefault_data[i % prefault_data_size].first);
 ...
 protected:
  typedef typename std::conditional<use_thp, utils::TransparentHugePageAllocator<PackedSoAKey<KeyT>>, std::allocator<PackedSoAKey<KeyT>>>::type
      PackedSoAKeyVectorAllocator;
  ...
  alignas(utils::cacheline_size) std::vector<PackedSoAKey<KeyT>, PackedSoAKeyVectorAllocator> keys_;
