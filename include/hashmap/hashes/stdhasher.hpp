...
struct StdHasher : public Hasher<KeyT, use_modulo> {
  StdHasher(uint64_t maximum_value) : Hasher<KeyT, use_modulo>(maximum_value) {}
  HEDLEY_ALWAYS_INLINE static uint64_t static_hash(const KeyT& key) { return std::hash<KeyT>()(key); }
  ...
  HEDLEY_ALWAYS_INLINE BucketHash<FingerprintT> bucket_hash(const KeyT& key) {
    ...
    static_assert(
        ...
        "For std::hash hashing, please do not use LSB/MSB, and use MSB/LSB instead. LSB/MSB is only relevant for multiply-shift types of hashing.");
